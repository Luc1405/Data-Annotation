...
CSV_PATH = INPUT_DIR / "pdok_coreconcept_annotations.csv"
DATASETS_DIR = INPUT_DIR / "pdok_data"
...
OUTPUT_CSV_PATH = OUTPUT_DIR / "pdok_coreconcept_annotations_output.csv"
OUTPUT_JSONL_PATH = OUTPUT_DIR / "pdok_coreconcept_annotations_output.jsonl"
...
def load_annotations(csv_path: Path) -> pd.DataFrame:
    """
    Loads the PDOK annotation input CSV.

    Required columns:
    Title, EnglishTitle, PageLink, MapLink, Kaartlaag, MapDescription

    Optional columns:
    Geometry, Entity

    Geometry and Entity are reference-label columns. For PDOK annotation-only runs
    these may be absent or empty, so they are created as empty columns when missing.
    """
    ...
    for optional_reference_column in ["Geometry", "Entity"]:
        if optional_reference_column not in df.columns:
            df[optional_reference_column] = pd.NA
...
def load_dataset_json(kaartlaag: str) -> dict[str, Any] | list[Any]:
    """
    Loads input_data/pdok_data/{Kaartlaag}.json by default.
    """
...
def ensure_output_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Geometry and Entity are optional reference-label columns and are kept intact when present.
    GPTGeometry and GPTEntity hold model predictions.
    """
...
Your task is to classify a geospatial map layer using exactly one Geometry type.
...
Your task is to classify a geospatial map layer using exactly one Entity type.
...
def write_run_artifacts(
    ...
    """
    Writes annotation artifacts for an unlabeled annotation run.

    This intentionally does not calculate accuracy, F1 scores, hierarchical scores,
    mismatch counts, or confusion matrices, because PDOK-derived input data may not have expert reference labels.
    """
    ...
    completed_rows = int((df["GPTGeometry"].notna() & df["GPTEntity"].notna() & df["GPTError"].isna()).sum())
    ...
    run_summary = {
        **run_metadata,
        ...
        "evaluation": {
            "enabled": False,
            "reason": (
                "No performance metrics were calculated because this input is intended "
                "for annotation only and does not require expert reference labels."
            ),
        },
    }
    ...
    shutil.copyfile(summary_json_path, RUN_SUMMARY_JSON_PATH)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Annotate geospatial datasets with LLM-generated semantic metadata."
    )
    parser.add_argument(
        "--run-id",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=CSV_PATH,
        help="Input CSV containing metadata rows to annotate.",
    )
    parser.add_argument(
        "--datasets-dir",
        type=Path,
        default=DATASETS_DIR,
        help="Directory containing PDOK GeoJSON files named {Kaartlaag}.json.",
    )
    parser.add_argument(
        "--provider",
...
def run_annotation_for_provider(
    ...
    run_summary = write_run_artifacts(df, run_dir, run_metadata)
    write_run_status(
        run_dir,
        provider_config,
        run_id,
        "completed",
        total_rows,
        int(run_summary["completed_rows"]),
        int(run_summary["error_rows"]),
    ...
    print(
        "Summary: "
        f"Completed={run_summary['completed_rows']}, "
        f"Errors={run_summary['error_rows']}, "
        f"Mean confidence={run_summary['mean_confidence']}"
    )
    return run_summary
...
def main() -> None:
    ...
    DATASETS_DIR = args.datasets_dir if args.datasets_dir.is_absolute() else BASE_DIR / args.datasets_dir

    print(f"Using input CSV: {input_csv_path}")
    print(f"Using PDOK data directory: {DATASETS_DIR}")
