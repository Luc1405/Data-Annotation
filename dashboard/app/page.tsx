      ...
      {baseRun && compareRun ? (
        <section className="card" style={{ marginBottom: 20 }}>
          ...
          <form className="actions">
            ...
            <select className="select" name="compare" defaultValue={compareRun.id}>
          ...
          <div className="table-wrap" style={{ marginTop: 16 }}>
            <table>
              ...
              <tbody>
                <tr><td>Geometry accuracy</td><td>{formatPercent(baseRun.geometry_accuracy)}</td><td>{formatPercent(compareRun.geometry_accuracy)}</td><td>{formatPercent((baseRun.geometry_accuracy ?? 0) - (compareRun.geometry_accuracy ?? 0))}</td></tr>
                <tr><td>Entity accuracy</td><td>{formatPercent(baseRun.entity_accuracy)}</td><td>{formatPercent(compareRun.entity_accuracy)}</td><td>{formatPercent((baseRun.entity_accuracy ?? 0) - (compareRun.entity_accuracy ?? 0))}</td></tr>
                <tr><td>Mean confidence</td><td>{formatNumber(baseRun.mean_confidence)}</td><td>{formatNumber(compareRun.mean_confidence)}</td><td>{formatNumber((baseRun.mean_confidence ?? 0) - (compareRun.mean_confidence ?? 0))}</td></tr>
              </tbody>
            </table>
          </div>
        </section>
